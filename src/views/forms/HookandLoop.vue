  ...
  <v-row justify="center">
    <v-dialog v-model="dialog" persistent max-width="600px">
      <template v-slot:activator="{ on, attrs }">
        <v-btn color="indigo darken-4" dark v-bind="attrs" v-on="on">Yeni Üretim Ekle</v-btn>
      </template>
      <v-card>
        <v-card-title>
          ...
          <v-container>
            <v-row>
              <v-col cols="12" sm="6" md="4">
                <v-text-field v-model="formHookandLoop.recordId" label="Record ID" required></v-text-field>
              </v-col>
              <v-col cols="12" sm="6" md="4">
              ...
              <v-col cols="12" sm="6" md="4">
                <v-text-field v-model="editedItem.spAdet" label="Sipariş Adet" required></v-text-field>
              </v-col>
              <v-col cols="12" sm="6">
                <v-select
              ...
              <v-col cols="12" sm="6">
                <v-select :items="['İnce', 'Kalın', 'Dijital']" label="Cırt Türü" required></v-select>
              </v-col>
              <v-col cols="12" sm="6">
                <v-select
              ...
              <v-col cols="12" sm="6">
                <v-select :items="['Süngersiz','3', '5']" label="Sünger (mm)" required></v-select>
              </v-col>
              <v-col cols="12" sm="6">
                <v-select :items="['Soft', 'Hard', 'CRT']" label="Taban" required></v-select>
              </v-col>
              <v-col cols="12">
        ...
        <v-card-actions>
          ...
          <v-btn color="blue darken-1" text @click="insertHookAndLoopGunlukImalat">Kaydet</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    <v-container>
...
export default {
  ...
  initialize() {},
  methods: {
    insertHookAndLoopGunlukImalat() {
      var formData = {
        recordId: this.formHookandLoop.recordId,
        isEmri: this.formHookandLoop.isEmri,
        musteri: this.formHookandLoop.musteri,
        cap: this.formHookandLoop.cap,
        spAdet: this.formHookandLoop.spAdet,
        tuyYuksekligi: this.formHookandLoop.tuyYuksekligi,
        deriTuru: this.formHookandLoop.deriTuru,
        deriAdet: this.formHookandLoop.deriAdet,
        cirtTuru: this.formHookandLoop.cirtTuru,
        cirtRengi: this.formHookandLoop.cirtRengi,
        sunger: this.formHookandLoop.sunger,
        taban: this.formHookandLoop.taban
      };
      this.$store.dispatch("insertHookAndLoopGunlukImalat", formData);
    }
  }
};
</script>